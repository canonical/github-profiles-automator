...
def test_httpbin_pebble_ready(harness: ops.testing.Harness[GithubProfilesAutomatorCharm]):
...
def test_config_changed_valid_can_connect(harness: ops.testing.Harness[GithubProfilesAutomatorCharm]):
...
def test_config_changed_valid_cannot_connect(harness: ops.testing.Harness[GithubProfilesAutomatorCharm]):
    # Trigger a config-changed event with an updated value
    harness.update_config({"log-level": "debug"})
...
def test_config_changed_invalid(harness: ops.testing.Harness[GithubProfilesAutomatorCharm]):
    # Ensure the simulated Pebble API is reachable
    harness.set_can_connect("httpbin", True)
    # Trigger a config-changed event with an updated value
