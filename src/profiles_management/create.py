...
from profiles_management.k8s.generic import get_name
from profiles_management.k8s.kfam import (
...
from profiles_management.k8s.profiles import list_profiles
...
def create_or_update_profiles(pmr: ProfilesManagementRepresentation):
    ...
    for existing_profile in existing_profiles:
        if pmr.has_profile(get_name(existing_profile)):
            continue

        logging.info("Profile %s not in PMR . Will remove access.", get_name(existing_profile))
        remove_access_in_stale_profile(existing_profile)

    # TODO: Create/Update Profiles based on the PMR
    # TODO: Cleanup RoleBindings in Profiles that don't match contributor in PMR
