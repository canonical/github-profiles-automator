...
def create_or_update_profiles(
    ...
    for profile_name, profile in pmr.profiles.items():
        ...
        rbs = kfam.delete_resources_not_matching_profile_contributors(client, profile, rbs)

        log.info("Deleting AuthorizationPolicies that don't match Profile Contributors.")
        aps = list_contributor_authorization_policies(client, profile.name)
        aps = kfam.delete_resources_not_matching_profile_contributors(client, profile, aps)
        ...
        log.info("Creating AuthorizationPolicies for Profile Contributors.")
        kfam.create_authorization_policies_for_profile_contributors(
            client, profile, aps, kfp_ui_principal, istio_ingressgateway_principal
        )
